...
function Navbar() {
  ...
  return (
    <nav
      className={color ? "bg-thick shadow-lg" : "bg-transparent shadow-none"}
    >
      <div className="container flex justify-between items-center font-text">
        <div className="text-xl text-primary font-semibold font-headings">
          Potted plants
        </div>
        <div className="hidden md:flex gap-6 font-semibold text-lg">
          {Menu.map((data) => (
            <NavLink
              ...
              className={({ isActive }) =>
                isActive
                  ? "text-primary shadow-[0_3px_0_#d8b97b]"
        ...
        <Link to="/products">
          <button className="hidden md:block text-lg py-1 px-3 rounded-2xl bg-secondary hover:bg-primary hover:text-secondary duration-100 border-2">
            Place an order
          </button>
        </Link>
        {open ? (
          <IoClose
            onClick={handleClick}
            className="text-xl md:hidden z-50 text-primary"
          />
        ) : (
          <IoMenu
            onClick={handleClick}
